...
--		1) DB schema name 
--			: CDM.dbo.measurement
--		2) concept_id, db_name을 짝맞춰서 변경
--			: 전체 6개 Table 생성

select * from cdm.dbo.measurement 
where measurement_concept_id = 3013721																		-- (concept_id 수정)
...
-- 3. 
IF OBJECT_ID('tempdb..#temp_3013721') IS NOT NULL															-- (concept_id 수정)
	DROP TABLE #temp_3013721																				-- (concept_id 수정)
...
IF OBJECT_ID('temp.dbo.person_AST') IS NOT NULL																-- (DB Name 수정)
	DROP TABLE temp.dbo.person_AST																			-- (DB Name 수정)

select person_id, measurement_concept_id, value_as_number, gender_source_value, age
into temp.dbo.person_AST																					-- (DB Name 수정)
from #temp_3013721																							-- (concept_id 수정)
